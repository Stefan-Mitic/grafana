		...
		t.Run(ttRaw.name, func(t *testing.T) {
			for _, tt := range splitTestcase(ttRaw) {
				t.Run(tt.name, func(t *testing.T) {
					...
					for _, d := range tt.dashboards {
						_, err := service.dashboardPermissions.SetPermissions(context.Background(), 1, d.UID, tt.dashboardPerms[d.UID]...)
						require.NoError(t, err)
					}
					...
					for i, r := range rules {
						...
						for _, p := range rperms {
							if permission := service.dashboardPermissions.MapActions(p); permission != "" {
